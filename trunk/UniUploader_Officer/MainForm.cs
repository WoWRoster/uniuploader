...
namespace WindowsApplication3
{
	...
	///	</summary>
	///
	public class MainForm : UniUploader_Officer.GUI_helper
	{
/*
...
*/
		...
		private UniUploader_Officer.http http = new UniUploader_Officer.http();
	...
	Keep My critical UniUploader_Officer settings updated: allows UA to set each setting within UU to certain values.  Also allows Image Logos to update from UA.
	Check for updates to UniUploader_Officer: allows UU to check for updates to UU when new releases come out (also allows you to auto-update UU).
	...
	Start with Windows: writes a registry key to your windows registry, causing windows to start UniUploader_Officer when you log in.
	...
	Download: manually initiates web=>wow";
		public string _HELP4 = @"If you need help setting up or troubleshooting UniUploader_Officer, or would like to make suggestions for UniUploader_Officer, requests for features, bug reports, or just chat, please visit http://wowroster.net or email me at mattm@wowroster.net (or Calystos at calystos@virgin.net)
UniUploader_Officer can be used for numerous types of WoW applications, and experimentation is encouraged.  Please show us your projects at http://wowroster.net
...
Special shout-out to the WoWRoster Dev Team! :)";
		...
		public string _CHECKSET = "Checking critical UniUploader_Officer settings.";
		public string _UPDWOWADDON = "Updating WoW Addons";
		public string _CHUUUPDATE = "Checking for UniUploader_Officer updates.";
...
If this does not take effect immediately, please restart the game after saving that setting.";
		...
		public string _NEWUPDAVAIL = "New Update for UniUploader_Officer available.";
		public string _UPDEXENOTFD = "Update_Officer.exe not found!";
		public string _DOWNUPD = "downloading update_Officer.exe";
		...
		public string _KMCUSU = "Keep My critical UniUploader_Officer settings updated";
		public string _CFUTU = "Check for updates to UniUploader_Officer";
		public string _AUTLU = "Automatically update to latest UniUploader_Officer";
...
The SV file is usually in DRIVE:\PROGRAM FILES\WORLD OF WARCRAFT\WTF\ACCOUNT\ACCOUNTNAME\";
		...
		private string _LOGINONCE = "You must log into World of Warcraft at least once more before running this program. please quit UniUploader_Officer and try again after you have logged into WoW";
		#endregion
		...
		private bool IsOfficerBuild = true;  // Set this to true to make sure this is a Officer/Leader only build
		private string OfficerStr = " Officer";  // Set this to "" if you do not wish to customize for a guild Officer build
...
// ----------------------------------------------
		...
		private void InitializeComponent()
		{
			...
			this.UUAutoUpdaterCheck.Text = "Automatically update to latest UniUploader_Officer";
			// 
			// UUUpdaterCheck
			// 
			...
			this.UUUpdaterCheck.Text = "Check for updates to UniUploader_Officer";
			...
			this.uuSettingsUpdater.Text = "Keep My critical UniUploader_Officer settings updated";
			...
			this.UpdatesURL.Text = "http://www.wowroster.net/UniUploader_Officer_updater2/update.php";
			// 
			// Options
			// 
...
// ----------------------------------------------
		...
		static void Main()
		{
			...
			// If UniUploader_Officer is already running then no point trying to launch a 2nd version
			...
			catch (Exception ex)
			{
				if (ex.GetType() == typeof(System.Security.SecurityException))
					MessageBox.Show("UniUploader_Officer is unable to run due to constraints set by the current .NET Framework security policy.", "Failure", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Stop);
				else
...
// ----------------------------------------------
		...
		private void Form1_Load(object sender, System.EventArgs e)
		{
			...
			http.onReceiveProgress += new UniUploader_Officer.http.ReceiveProgressDelegate(http_onReceiveProgress);
			http.onSendProgress += new UniUploader_Officer.http.SendProgressDelegate(http_onSendProgress);
			http.onInformationMessage += new UniUploader_Officer.http.httpInfoDelegate(http_onInformationMessage);
...
// ----------------------------------------------
		...
		private void doScreenshots()
		{
			...
			UniUploader_Officer.http.Response Response = new UniUploader_Officer.http.Response();
		...
		private void WriteSettings()
		{
			try
			{
				if (windowstate != "starting")
				{
					...
					#endregion
					IniStructure.WriteIni(ini, WorkSpacePath + "\\settings.ini", "UniUploader_Officer initialization file \ngenerated by \n" + "Version : " + uniVersionMajor + "." + uniVersionMinor + "." + uniVersionBuild + "." + uniVersionRevision);
				}
			}
			catch (Exception e)
			{
		...
		private string InstallStartupKey()
		{
			try
			{
				...
				if (key != null)
				{
					...
					if (key.GetValue("UniUploader_Officer") != null)
					{
						//key exists, check it
						string pathFromReg = (string)key.GetValue("UniUploader_Officer");
						if (pathFromReg != path + "\\UniUploader_Officer.exe")
						{
							//key is wrong, change it
							key.SetValue("UniUploader_Officer", path + "\\UniUploader_Officer.exe");
							return _STVALFIX;
						}
						else
						{
					...
					{
						// UniUploader_Officer key doesent exist, create it
						key.SetValue("UniUploader_Officer", path + "\\UniUploader_Officer.exe");
						return _STVALCREATE;
					}
				}
			...
			try
			{
				...
				if (key != null)
				{
					...
					if (key.GetValue("UniUploader_Officer") != null)
					{
						...
						key.DeleteValue("UniUploader_Officer", false);
						return _STVALDEL;
					}
					else
					{
						// UniUploader_Officer key doesent exist *shrug*
						return _STVALNOTFD;
					}
				}
				else
...
// ----------------------------------------------
		...
		private void linkLabel1_LinkClicked(object sender, System.Windows.Forms.LinkLabelLinkClickedEventArgs e)
		{
			...
			System.Diagnostics.Process.Start("mailto:Matt@MatthewMiller.info?subject=UniUploader_Officer");
		}
...
// ----------------------------------------------
		...
		private void linkLabel5_LinkClicked(object sender, System.Windows.Forms.LinkLabelLinkClickedEventArgs e)
		{
			...
			System.Diagnostics.Process.Start("mailto:calystos@virgin.net?subject=UniUploader_Officer");
		}
...
// ----------------------------------------------
		...
		public void UpdateUUSettings2()
		{
			...
			bool oldDelims = true;
			if (uaVersion != "UniUploader_Officer Update Interface Ready..." && uaVersion != "")
			{
				oldDelims = false;
			}
			string[] settings;
...
// ----------------------------------------------
		...
		public bool CheckVersion(String verToCheck)
		{
			...
			{ // Alpha digit found in version EG 0.0.0a and not 0.0.0, use old style version checker
				...
				{
					DebugLine("New UniUploader_Officer version detected");
					return true;
				}
			}
				...
				DebugLine("UniUploader_Officer Version: " + v1 + ", Update.php UniUploader_Officer Version: " + v2 + ".");
				if (v1 >= v2) { DebugLine("UniUploader_Officer is latest version"); return false; }
			...
			DebugLine("UniUploader_Officer is latest version");
			return false;
		}
...
// ----------------------------------------------
		...
		{
			string UpdateQueryResponse = "";
			UpdateQueryResponse = RetrData(UpdatesURL.Text, null, null, "OPERATION", "CHECKUPDATESOFFICER", null, null, null, null, UUTimeOut, null, null);
			char[] sep = { '|' };
			...
			else
			{
				...
				if (CheckVersion(Response[0]) && !UserClickedNoUpdate)
				{
					if (UUAutoUpdaterCheck.Checked)
					{
						...
						//updater retrieval
						UpdateQueryResponse = RetrData(UpdatesURL.Text, null, null, "OPERATION", "GETUPDATEROFFICERLATEST", null, null, null, null, -1, null, null);
						...
						//UpdateLanguagePack();
						//PopulateLanguageSelector();
						if (File.Exists(UUPath + "\\update_Officer.exe"))
						{
							updating = true;
							System.Diagnostics.ProcessStartInfo start = new System.Diagnostics.ProcessStartInfo(UUPath + "\\update_Officer.exe", @"-uuver """ + uniVersionMajor + "." + uniVersionMinor + "." + uniVersionBuild + "." + uniVersionRevision + @""" -lang """ + Language + @"""");
							System.Diagnostics.Process.Start(start);
							UpdateBypass = true;
							CloseForm();
						}
...
// ----------------------------------------------
		...
		private bool showLicense(bool buttons)
		{
			...
			licenseForm.Text = "UniUploader_Officer License";
			licenseForm.Show();
...
// ----------------------------------------------
		...
		{
			return "UniUploader_Officer " + uniVersionMajor + ".0 (UU " + uniVersionMajor + "." + uniVersionMinor + "." + uniVersionBuild + "." + uniVersionRevision + "; " + Language + ")";
		}
...
// ----------------------------------------------
		...
		public string UploadALL(string url, string fieldname, CookieContainer cookies, CredentialCache credentials, string param1, string param2, string param3, string param4, string val1, string val2, string val3, string val4, string name, string location)
		{
			...
			UniUploader_Officer.http.FILE_COMPRESSION_METHODS CompressionMethod = UniUploader_Officer.http.FILE_COMPRESSION_METHODS.NONE;
			if (GZcompress.Checked)
				CompressionMethod = UniUploader_Officer.http.FILE_COMPRESSION_METHODS.GZIP;
			http.UserAgent = getUserAgent();

			DebugLine("UploadALL: ------------------------------------------------------------------------");
			//http.onInformationMessage += new UniUploader_Officer.http.httpInfoDelegate(http_onInformationMessage);
			//string result = http.post(files, AllParams, url, UniUploader_Officer.http.ENCODING_TYPES.MULTI_FORM_DATA, UniUploader_Officer.http.REQUEST_METHODS.POST, "", CompressionMethod, cookies, credentials);
