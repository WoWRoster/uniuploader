...
namespace update_Officer
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.Timer timer1;  // Retry timer if UniUploader_Officer is still running
...
// ----------------------------------------------
		...
		private void InitializeComponent()
		{
			...
			this.label1.Text = "Downloading New Version of UniUploader_Officer.";
			// 
			// progressBar1
			// 
...
// ----------------------------------------------
		...
		static void Main(string[] Args) 
		{
			...
			{
					MessageBox.Show("Please run the update process from within UniUploader_Officer.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
					Application.Exit();
			}
		}
...
// ----------------------------------------------
		...
		private void Form1_Load(object sender, System.EventArgs e)
		{
			...
			DebugLine("Waiting for UniUploader_Officer to close before initiating download.");
		}
...
// ----------------------------------------------
		...
		public void timer2_Tick(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				numLaunchChecks = 0;
				DebugLine("Automatically launching new UniUploader_Officer version.");
				...
				System.Diagnostics.ProcessStartInfo start = new System.Diagnostics.ProcessStartInfo(UUPath+"\\UniUploader_Officer.exe");
...
// ----------------------------------------------
		...
		public void updateNOW()
		{
			...
			UpdateQueryResponse = RetrData(UpdatesURL,null,null,"OPERATION","GETLATESTOFFICER","LANGUAGE",UULanguage,null,null,20000);
			...
			DebugLine("New UniUploader_Officer.exe downloaded, Please press the \"Launch New Version\" Button.");
...
// ----------------------------------------------
		...
		public string RetrData(string url, CookieContainer cookies, CredentialCache credentials, string param1, string val1, string param2, string val2, string param3, string val3, int Timeout) 
		{
			...
			req.Method = "POST";
			req.UserAgent = "UniUploader_Officer "+updver_major + ".0 (UU " + updver_major+ "." + updver_minor + "." + updver_build + "." + updver_revision + "; " + UULanguage+")";
			if (Timeout > 0)
			{
				req.Timeout = Timeout;
			}
		...
		public void timer1_Tick(object sender, System.EventArgs e)
		{
			...
			if (numChecks <= 10)
			{
				...
				else
				{
					...
					DebugLine("UniUploader_Officer window not found.");
					if (isUUWritable())
					{
						DebugLine("UniUploader_Officer.exe is now unlocked.");
						DebugLine("Commencing Upgrade.");
						startDownload();
					}
					else
					{
						DebugLine("UniUploader_Officer.exe is still Locked.");
						timer1.Enabled = true;
						timer1.Start();
					}
				}
			...
			else
			{
				timer1.Stop();
				timer1.Enabled = false;
				DebugLine("Waited 10 seconds for UniUploader_Officer to close. Forcing check.");
				if (isUUWritable())
				{
					DebugLine("UniUploader_Officer.exe is now unlocked.");
					DebugLine("Commencing Upgrade.");
...
// ----------------------------------------------
		...
		public bool isUUWritable()
		{
			try
			{
				FileStream fs = new FileStream("UniUploader_Officer.exe", FileMode.OpenOrCreate,FileAccess.Write);
				if (fs.CanWrite)
				{
					fs.Close();
					return true;
